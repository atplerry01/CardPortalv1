...
class CmpTableWithCheckBox extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    let tableItems = formatedBody.map((item, index) => {
      let isChecked = false;
      let hasItem = selectedCheckedItems !== null ?  selectedCheckedItems.find(
      ...
      return (
        <tr key={index}>
          ...
          {row}
          <td onClick={this.onNavClick.bind(this, item)} style={{ cursor: 'pointer' }}>
            {/* <i className="fa fa-pencil-square-o" aria-hidden="true"></i>View */}
          </td>
          {onEditClicked && (
            <td>
              <button type="button" onClick={(e) => { this.onEditClicked(e, item) }} className="btn btn-primary-outline">
          ...
          {onDeleteClicked && (
            <td>
              <button type="button" onClick={(e) => { this.onDeleteClicked(e, item) }} className="btn btn-primary-outline">
    ...
    if (selectedCheckedItems !== null && selectedCheckedItems.length === dataBody.length) {
      isChecked = true;
    }
...
CmpTableWithCheckBox.propTypes = {
...
export default CmpTableWithCheckBox;
