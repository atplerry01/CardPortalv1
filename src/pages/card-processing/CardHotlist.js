...
export default class CardHotlist extends Component {
    ...
    handleCardHostList = async entity => {
        ...
        if (entity && entity.cardStatus === "Active") {
            this.setState({ showModal: true, processHotList: true, selectedHostlist: entity });
        }
    };
    ...
    cancelProcess = async e => {
        e.preventDefault();
        window.location = "cards/cardhotlist";
    };
    ...
    render() {
        const { accountNumber, isSearchStage, isSubmitStage, hasAccount } = this.state;

        return (
            <>
                <ContentHeader title="Cards Hotlist" />
    ...
    renderModalContent = () => {
        ...
        return (
            <ExistingCardTable processing={processing} cards={cards} handleAction={this.handleCardHostList} isPage={true} />
        );
    };
    ...
    getAccountQuery = async accountNumber => {
        return await issuanceService.instanceHotListQueryService(accountNumber)
            .then(res => {
                return res.data;
            },
